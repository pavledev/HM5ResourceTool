#include "pch.h"
#include <format>
#include "FSB/Vorbis/ZRebuilder.hpp"

namespace FSB
{
    namespace Vorbis
    {
        ZRebuilder::ZRebuilder()
        {

        }

        void ZRebuilder::Rebuild(const Sample& sample, IO::ZBufferView& sampleView, std::ostream& stream)
        {
            ...
            OGGOuputStream oggStream(1, stream);

            {
                ...
                oggStream.WritePacket(headerID);
                oggStream.WritePacket(headerComment);
                oggStream.WritePacket(headerSetup);
                oggStream.FlushPackets();
...

            {
                ...
                while (packetSize)
                {
                    ...
                    packet.granulepos = prevBlocksize ?
                        prevGranulepos + (blocksize + prevBlocksize) / 4 : 0;

                    oggStream.WritePacket(packet);
        ...
        void ZRebuilder::Rebuild(const Sample& sample, IO::ZBufferView& sampleView, HM5ResourceTool::IO::ZBinaryWriter& binaryWriter)
        {
            VorbisInfoHolder info;
            VorbisCommentHolder comment;
        ...
        void ZRebuilder::RebuildHeaders(
        ...
        void ZRebuilder::RebuildIDHeader(
        ...
        void ZRebuilder::RebuildCommentHeader(
            OGGPacketHolder& packet,
        ...
        void ZRebuilder::RebuildSetupHeader(
